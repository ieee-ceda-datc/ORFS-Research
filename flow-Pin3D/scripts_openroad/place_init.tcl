...
# puts "Reading DEF file: $env(RESULTS_DIR)/$env(DESIGN_NAME)_$env(DESIGN_DIMENSION).gp.def"
# read_def -floorplan_initialize $env(RESULTS_DIR)/$env(DESIGN_NAME)_$env(DESIGN_DIMENSION).gp.def
# source $env(PLATFORM_HOME)/$env(PLATFORM)/setRC.tcl

read_def $env(RESULTS_DIR)/$env(DESIGN_NAME)_$env(DESIGN_DIMENSION).gp.def
...
write_def $env(RESULTS_DIR)/$env(DESIGN_NAME)_$env(DESIGN_DIMENSION).gp.def
# write_verilog $env(RESULTS_DIR)/$env(DESIGN_DIMENSION).gp.v

save_image -resolution 0.1 $::env(RESULTS_DIR)/place_init.webp 
